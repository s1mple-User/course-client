...
export const pushStudentCourse = createAsyncThunk<any, any>(
	'admin/push-student',
	async (body, thunkApi) => {
		try {
			const response = await AdminService.pushStudentCourse(body.courseId,body.email);
			body.callback();
			return response;
		} catch (error) {
			return thunkApi.rejectWithValue(errorCatch(error));
